...
# NEW FOR LAB 10: import uprotobuf-generated classes
from temp_message_upb2 import TempReading, Time as ProtoTime
...
# ============================================================
# PUBLISHER: TEMPERATURE READING (NOW WITH PROTOBUF)
# ============================================================
...
def make_proto_payload(temp):
    """
    Build a TempReading protobuf message and serialize it.
    """
    msg = TempReading()

    # publisher_id (schema is string, so decode PUB_IDENT_B if needed)
    if isinstance(PUB_IDENT_B, bytes):
        msg.publisher_id._value = PUB_IDENT_B.decode()
    else:
        msg.publisher_id._value = PUB_IDENT_B

    # temperature (float)
    msg.temperature._value = float(temp)

    # time (hour, minute, second)
    tstruct = time.localtime()   # (year, month, mday, hour, minute, second, weekday, yearday)
    tmsg = ProtoTime()
    tmsg.hour._value = tstruct[3]
    tmsg.minute._value = tstruct[4]
    tmsg.second._value = tstruct[5]

    msg.time._value = tmsg

    # Convert to bytes for MQTT
...
def publish_temp(timer):
    ...
    try:
        ...
        print("Publishing protobuf payload (len={} bytes) to topic: {}".format(len(payload), TOPIC_B))
...
# ============================================================
# SUBSCRIBER: STATE TRACKING & OUTPUT (PROTOBUF PARSING)
# ============================================================
...
def mqtt_callback(topic, message):
    ...
    try:
        ...
        # Extract fields using ._value (uProtoBuf API)
        pub_id_str = msg.publisher_id._value
        temp_val = msg.temperature._value

        # Time fields (not used in logic here, but parsed for completeness)
